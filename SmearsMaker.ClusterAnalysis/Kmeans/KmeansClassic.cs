...
namespace SmearTracer.ClusterAnalysis.Kmeans
{
	public sealed class KmeansClassic : Kmeans
	{
		...
		public override List<Point> GetClusteredPoints()
		{
			...
			foreach (var cluster in Clusters)
			{
				...
				foreach (var data in cluster.Data)
				{
					...
					var point = new Point(data.Original, data.Position.X, data.Position.Y);
					point.SetFilteredPixel(pixel.Data);
		...
		public List<Point> GetAveragePoints()
		{
			...
			foreach (var cluster in Clusters)
			{
				var average = new float[cluster.Data[0].Original.Length];
				foreach (var point in cluster.Data)
				{
					var data = point.Original.Data;
					for (int i = 0; i < point.Original.Length; i++)
					{
						average[i] += data[i];
					}
				}
				for (int i = 0; i < cluster.Data[0].Original.Length; i++)
				{
				...
				{
					var point = new Point(data.Original, data.Position.X, data.Position.Y);
					point.SetFilteredPixel(average);
					clusteringPoints.Add(point);
				}
			}
		...
		{
			var sortedArray = Points.OrderBy(p => p.Filtered.Sum).ToArray();
			...
			for (int i = 0; i < Clusters.Count; i++)
			{
				...
				Clusters[i].Centroid = point.Filtered;
			}
		}
		protected override void UpdateCentroid(Cluster cluster)
		{
			...
			foreach (var data in cluster.Data)
			{
				var dataArray = data.Filtered.Data;

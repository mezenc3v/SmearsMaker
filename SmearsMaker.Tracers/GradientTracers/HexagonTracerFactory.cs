...
{
	public class HexagonTracerFactory : IServicesFactory
	{
		private readonly ImageModel _model;
		private readonly IProgress _progress;
		private readonly GtImageSettings _settings;

		public HexagonTracerFactory(GtImageSettings settings, ImageModel model, IProgress progress)
		{
			_model = model;
			_progress = progress;
			_settings = settings;
		...
		{
			return new HexagonSplitter(_progress, (int)_settings.WidthSmear.Value);
		}
